...
class RemindersService: ObservableObject {
    ...
    private let reminderListName = "AssetManager"
    ...
    func syncRecordToReminders(_ record: macOS_OperationRecord) async -> Bool {
        ...
        let alreadyExists = existingReminders.contains { reminder in
            reminder.title?.contains(record.assetName) == true
        }
        ...
        reminder.title = "📦 归还提醒: \(record.assetName)"
        reminder.calendar = calendar
        
        // 设置备注
        var noteComponents: [String] = []
        noteComponents.append("条码: \(record.assetId)")
        noteComponents.append("操作人: \(record.operatorName)")
        noteComponents.append("出库时间: \(Self.formatDateTime(record.timestamp))")
        if let note = record.note {
            noteComponents.append("备注: \(note)")
        }
    ...
    /// 批量同步出库记录到提醒事项
    /// 返回成功同步的记录ID列表
    func syncCheckOutRecords(_ records: [macOS_OperationRecord]) async -> [UUID] {
        isSyncing = true
        syncMessage = nil
        ...
        if successCount > 0 {
            syncMessage = "已同步 \(successCount) 条记录到提醒事项"
        } else {
            syncMessage = "没有需要同步的记录"
        }
